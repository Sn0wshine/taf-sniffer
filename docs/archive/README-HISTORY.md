...
Taf Sniffer est un outil open source qui aide les personnes en reconversion à trouver, comparer et comprendre des offres d'emploi selon leur projet professionnel.

Le projet combine recherche d'offres, import manuel, analyse locale explicable et classement assisté par IA. Il est conçu pour garder l'humain au centre de la décision : Taf Sniffer aide à lire et prioriser les annonces, mais ne remplace ni la vérification de l'offre ni le jugement du candidat.

> **Statut : MVP open source.** Le projet est actif et évolue rapidement. Les connecteurs de sites publics sont best-effort ; l'import manuel reste le parcours le plus fiable.

## Démo

Une courte démonstration montrant la recherche, le classement et l'import manuel est décrite dans [le storyboard de démo](docs/DEMO.md). Le parcours complet est reproductible localement en quelques commandes ci-dessous.
...
Copier `.env.example` vers `.env` uniquement si une configuration serveur est nécessaire. Les clés API ne doivent jamais être ajoutées au dépôt.

- **Sans clé API**, l'analyse locale et l'import manuel restent disponibles.
- **Gemini** active le classement IA et les comparaisons assistées lorsque la clé est configurée.
- **France Travail** peut utiliser l'API officielle avec des identifiants développeur dédiés.

Les recherches automatiques interrogent plusieurs sources publiques lorsque cela est possible. Le HTML, les robots, les quotas et les conditions d'utilisation de ces sites peuvent changer. Utiliser l'import manuel lorsque la collecte automatique est bloquée et respecter les conditions de chaque service.

## Pour contribuer

Le code client se trouve dans `src/`, le proxy local dans `server/` et les tests dans `src/__tests__/` et `server/__tests__/`. Les règles de développement, de pull request et de signalement de vulnérabilité sont décrites dans [CONTRIBUTING.md](CONTRIBUTING.md) et [SECURITY.md](SECURITY.md).

Les contributions sont particulièrement utiles pour les tests, l'accessibilité, les connecteurs, la documentation et l'amélioration des règles de classement.
...
Le build de production est vérifié par `npm run build` et produit le dossier `dist`, utilisé par Capacitor.
